package com.gary.garytool.lib.fancyindexer.ui;
...
//	int  mTipFontColor = 0xff3399ff;
	...
	float mLastOffset[] = new float[ConstChar.length]; // 记录每一个字母的x方向偏移量, 数字<=car_brand_0
	...
	Handler mHideWaitingHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			if( msg.what == 1 )
			{
//				mScroller.startScroll(car_brand_0, car_brand_0, 255, car_brand_0, 1000);
				mHideAnimation = true;
				mAnimating = false;
...
//	    	   canvas.drawLine(car_brand_0, yPos, width, yPos, mPaint);
	       }
	       mPaint.reset();
	    }
	...
	/**
	 * 计算出所有贝塞尔曲线上的点 
	 * 个数为 mMaxBezierLines * car_brand_2 = 64
	 */
